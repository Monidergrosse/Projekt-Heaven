...
class VehicleSpawner {
  ...
  static show() {
    ...
    content.push(createTextInput("vehicleSearchBar", "Search", (input) => search(input.toLocaleLowerCase())));

    let searchCategory = createText(undefined, "Search", () => {});
    let searchCategoryBr1 = document.createElement('br');
    let searchCategoryBr2 = document.createElement('br');
    searchCategory.style.display = "none";
    searchCategoryBr1.style.display = "none";
    searchCategoryBr2.style.display = "none";
    modelsContent.push(searchCategoryBr1);
    modelsContent.push(searchCategory);
    modelsContent.push(searchCategoryBr2);

    models.forEach(model => {
      if(lastCategory != model.category) {
        modelsContent.push(document.createElement('br'));
        modelsContent.push(createText("categorieName", categories[model.category], () => {}));
        modelsContent.push(document.createElement('br'));
      ...
      icon.src = "img/VehicleSpawner/models/" + model.model + ".jpg";
      modelContent.push(icon);

      let modelText = createText("modelName", model.model, () => {});
      modelContent.push(modelText);

      let modelElement = createContainer("model", modelContent);
      modelElement.name = model.model;
      modelElement.category = model.category;
      modelElement.addEventListener("mouseenter", () => {
        modelElement.style.margin = "1px";
        modelElement.style.width = "calc(25% - 10px + 8px)";
        modelElement.style.border = "solid 1px #A2A9AF";
        modelElement.style.backgroundColor = "lightGreen";
        modelText.innerHTML = "Spawn";
      });
      modelElement.addEventListener("mouseleave", () => {
        modelElement.removeAttribute("style");
        modelText.innerHTML = modelElement.name;
      });
      modelElement.addEventListener("click", () => alt.emit("VehicleSpawner:spawn", modelElement.name));
      modelsContent.push(modelElement);
    });

    content.push(createContainer("modelContainer", modelsContent));

    document.body.appendChild(createUiPanel("vehicleSpawner", "Vehicle Spawner", content));
    ...
    function search(input) {
      ...
      if(input.startsWith("@"))
        searchByCategory(input.substring(1));
      else
        searchByName(input);
      ...
      {
        Array.from(document.getElementsByClassName("modelContainer")[0].getElementsByTagName("div")).forEach(element => {
      ...
      {
        Array.from(document.getElementsByClassName("modelContainer")[0].getElementsByTagName("div")).forEach(element => {
          if(categories[element.category].toLocaleLowerCase().startsWith(category))
            element.style.display = "";
          else
            element.style.display = "none";
        });

        Array.from(document.getElementsByName("categorieName")).forEach(element => {
          if(element.innerHTML.toLocaleLowerCase().startsWith(category)) {
          	element.style.display = "";
      ...
      function hideCategories() {
        Array.from(document.getElementsByName("categorieName")).forEach(element => element.style.display = "none");
        Array.from(document.getElementsByClassName("modelContainer")[0].getElementsByTagName("br")).forEach(element => element.style.display = "none");
        searchCategory.style.display = "";
        searchCategoryBr1.style.display = "";
        searchCategoryBr2.style.display = "";
      }

      function showCategories() {
        Array.from(document.getElementsByName("categorieName")).forEach(element => element.style.display = "");
        Array.from(document.getElementsByClassName("modelContainer")[0].getElementsByTagName("br")).forEach(element => element.style.display = "");
        searchCategory.style.display = "none";
        searchCategoryBr1.style.display = "none";
        searchCategoryBr2.style.display = "none";
      }
