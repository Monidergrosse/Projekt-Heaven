...
alt.on('keydown', async (key) => {
	if (key == "G".charCodeAt(0)) {
		if (MugshotBoard.active) {
			...
			let fullname = await getDatabaseVariable("character.fullname");
			await MugshotBoard.start("", "", fullname, "");
		}
	}
});
...
function getDatabaseVariable(variable) {
	return new Promise((reslove, reject) => {
		...
		alt.emitServer('getDatabaseVariable', token, variable);
		const setDatabaseVariable = (_token, value) => {
			if (token != _token) return;
			alt.offServer('setDatabaseVariable', setDatabaseVariable);
			reslove(value);
		};
		alt.onServer('setDatabaseVariable', setDatabaseVariable);
	});
}
